################ 2x T4 #######################
import sys
path_code = '/kaggle/input/bratscode'
sys.path.append(path_code)
from torch.utils.data import DataLoader
import torch
from T4x2_NVNSWASRCBGAMHCRFB import T4x2_NVNSWASRCBGAMHCRFB

from dataset import BRATS
from loss import MHLoss_1,MHLoss_2
from DataAugmentationBlock import DataAugmenter
...
loss_choice_str = 'MHLoss_1'
model_choice = {
    'T4x2_NVNSWASRCBGAMHCRFB':T4x2_NVNSWASRCBGAMHCRFB
}
model_string = 'T4x2_NVNSWASRCBGAMHCRFB'
csv_dir = '/kaggle/input/fold01234/brats2021_split_fold0.csv'
root_dir = '/kaggle/working/brain_images'
data_train = BRATS(
    csv_dir=csv_dir,
    mode='train',
    root_dir=root_dir
...
if AUGMENTATION:
    data_augmentation = DataAugmenter(p=0.8, noise_only=False, channel_shuffling=False, drop_channnel=True).to('cuda:0')
if GAN_TRAINING:
    #model = GAN3D(4,3,device,0.1,model_string,data_augmentation)
    pass
else:
    model = model_choice[model_string](4,3,0.75)
    ...
    for epoch in range(epochs):
        ...
        if not GAN_TRAINING:
            ...
            for i,data in enumerate(tqdm(train_dataloader)):
                ...
                optim.zero_grad()
                outputs = model(inputs)
                loss_cal = loss_func(outputs,inputs.to('cuda:1'),label.to('cuda:1'))
