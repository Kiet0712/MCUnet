...
from pixelShuffle import ConvWithPixelShuffle
...
class Up(nn.Module):
    def __init__(self,in_channels,out_channels):
        super().__init__()
        self.up = ConvWithPixelShuffle(in_channels,in_channels//2,2)
...
class MP2(nn.Module):
    def __init__(self, scale) -> None:
        ...
        self.conv4 = ConvWithPixelShuffle(int(512*scale),int(64*scale),2)
    def forward(self,x1,x2,x3,x4):
        x_1_path = self.conv1(x1)
        x_2_path = self.conv2(x2)
        x_3_path = self.conv3(x3)
...
class MP3(nn.Module):
    def __init__(self, scale) -> None:
        ...
        self.conv3 = ConvWithPixelShuffle(int(256*scale),int(32*scale),2)
        self.conv4 = nn.Sequential(
            ConvWithPixelShuffle(int(512*scale),int(16*scale),2),
            nn.InstanceNorm3d(int(16*scale)),
            nn.ReLU(inplace=True),
            ConvWithPixelShuffle(int(16*scale),int(32*scale),2)
        )
    def forward(self,x1,x2,x3,x4):
        x_1_path = self.conv1(x1)
        x_2_path = self.conv2(x2)
...
class MP4(nn.Module):
    def __init__(self,scale) -> None:
        ...
        self.conv2 = ConvWithPixelShuffle(int(128*scale),int(16*scale),2)
        self.conv3 = nn.Sequential(
            ConvWithPixelShuffle(int(256*scale),int(8*scale),2),
            nn.InstanceNorm3d(int(8*scale)),
            nn.ReLU(inplace=True),
            ConvWithPixelShuffle(int(8*scale),int(16*scale),2)
        )
        self.conv4 = nn.Sequential(
            ConvWithPixelShuffle(int(512*scale),int(8*scale),2),
            nn.InstanceNorm3d(int(8*scale)),
            nn.ReLU(inplace=True),
            ConvWithPixelShuffle(int(8*scale),int(8*scale),2),
            nn.InstanceNorm3d(int(8*scale)),
            nn.ReLU(inplace=True),
            ConvWithPixelShuffle(int(8*scale),int(16*scale),2)
        )
...
class CRFB(nn.Module):
    def __init__(self,top_channels,bottom_channels):
        ...
        self.deconv = nn.Sequential(
            ...
            nn.ReLU(inplace=True),
            ConvWithPixelShuffle(bottom_channels//4,top_channels,2)
        )
...
class PSNVNSWASRCBGAMHCRFB(nn.Module):
    def __init__(self, n_channels, n_classes,scale=0.5):
        super(PSNVNSWASRCBGAMHCRFB, self).__init__()
