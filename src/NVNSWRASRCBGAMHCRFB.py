...
class OutConv(nn.Module):
    def __init__(self, in_channels, out_channels,n_channels):
        ...
        self.attention_class_1 = Attention_block(n_channels*2,in_channels,in_channels//2)
        self.attention_class_2 = Attention_block(n_channels*2,in_channels,in_channels//2)
        self.attention_class_4 = Attention_block(n_channels*2,in_channels,in_channels//2)
        self.attention_reconstruct = Attention_block(n_channels,in_channels,in_channels//2)
    def forward(self, x):
        reconstruct_volume = self.reconstruct_volume_conv(x)
        mask_head = self.mask_head(torch.cat([self.attention_reconstruct(reconstruct_volume,x),reconstruct_volume],dim=1))
        ...
        class_1_segment_vol = self.class_1_segment_conv(torch.cat([self.attention_class_1(class_1_guide,x),class_1_guide],dim=1))
        class_2_segment_vol = self.class_2_segment_conv(torch.cat([self.attention_class_2(class_2_guide,x),class_2_guide],dim=1))
        class_4_segment_vol = self.class_4_segment_conv(torch.cat([self.attention_class_4(class_4_guide,x),class_4_guide],dim=1))
...
class NVNSWRASRCBGAMHCRFB(nn.Module):
    def __init__(self, n_channels, n_classes,scale=0.5):
        super(NVNSWRASRCBGAMHCRFB, self).__init__()
