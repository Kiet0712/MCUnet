...
class T4x2_NVNSWASRCBGAMHCRFB_P1(nn.Module):
    def __init__(self, n_channels, n_classes, scale=0.5):
        super(T4x2_NVNSWASRCBGAMHCRFB_P1, self).__init__()
    ...
    def forward(self, x):
        ...
        return x, x_1_res_path,x_2_res_path,x_3_res_path,x_4_res_path
class T4x2_NVNSWASRCBGAMHCRFB_P2(nn.Module):
    def __init__(self, n_channels, n_classes, scale=0.5):
        super(T4x2_NVNSWASRCBGAMHCRFB_P2, self).__init__()
        self.n_channels = n_channels
        self.n_classes = n_classes
    ...
    def forward(self, x, x_1_res_path,x_2_res_path,x_3_res_path,x_4_res_path):
        x = self.up4(x, self.MP4(x_1_res_path,x_2_res_path,x_3_res_path,x_4_res_path))
        output = self.outc(x)
        return output
class T4x2_NVNSWASRCBGAMHCRFB(nn.Module):
    def __init__(self, n_channels, n_classes, scale=1):
        assert torch.cuda.is_available()==True, "Check 2x T4"
        assert torch.cuda.device_count()==2, "Check 2x T4."
        super(T4x2_NVNSWASRCBGAMHCRFB, self).__init__()
        self.n_channels = n_channels
        self.n_classes = n_classes

        self.p1 = T4x2_NVNSWASRCBGAMHCRFB_P1(n_channels, n_classes, scale).to('cuda:0')
        self.p2 = T4x2_NVNSWASRCBGAMHCRFB_P2(n_channels, n_classes, scale).to('cuda:1')
    def forward(self, x):
        x = x.to('cuda:0')
        x1, x2, x3, x4, x5 = self.p1(x)
        x1 = x1.to('cuda:1')
        x2 = x2.to('cuda:1')
        x3 = x3.to('cuda:1')
        x4 = x4.to('cuda:1')
        x5 = x5.to('cuda:1')
        return self.p2(x1, x2, x3, x4, x5)
