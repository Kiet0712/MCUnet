...
class OutConv(nn.Module):
    def __init__(self, in_channels, out_channels,n_channels):
        ...
        self.class_1_segment_conv = MDynamic_conv3d(in_channels,out_channels//3,1)
        self.class_2_segment_conv = MDynamic_conv3d(in_channels,out_channels//3,1)
        self.class_4_segment_conv = MDynamic_conv3d(in_channels,out_channels//3,1)
        self.class_1_variant_guide = nn.Sequential(
            nn.Conv3d(n_channels*2,in_channels,1),
            nn.BatchNorm3d(in_channels)
        )
        self.class_2_variant_guide = nn.Sequential(
            nn.Conv3d(n_channels*2,in_channels,1),
            nn.BatchNorm3d(in_channels)
        )
        self.class_4_variant_guide = nn.Sequential(
            nn.Conv3d(n_channels*2,in_channels,1),
            nn.BatchNorm3d(in_channels)
        )
    ...
    def forward(self, x):
        ...
        class_1_guide = self.class_1_variant_guide(torch.cat([mask_head[:,0:4,:,:,:],mask_head[:,12:16,:,:,:]],dim=1))
        class_2_guide = self.class_2_variant_guide(torch.cat([mask_head[:,4:8,:,:,:],mask_head[:,16:20,:,:,:]],dim=1))
        class_4_guide = self.class_4_variant_guide(torch.cat([mask_head[:,8:12,:,:,:],mask_head[:,20:,:,:,:]],dim=1))
        class_1_segment_vol = F.sigmoid(self.class_1_segment_conv(x,class_1_guide))
        class_2_segment_vol = F.sigmoid(self.class_2_segment_conv(x,class_2_guide))
        class_4_segment_vol = F.sigmoid(self.class_4_segment_conv(x,class_4_guide))
...
class NSWSRCBMDCGAMHCRFB(nn.Module):
    def __init__(self, n_channels, n_classes,scale=0.5):
        super(NSWSRCBMDCGAMHCRFB, self).__init__()
