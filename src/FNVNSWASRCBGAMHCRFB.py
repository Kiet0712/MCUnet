...
class OutConv(nn.Module):
    def __init__(self, in_channels, out_channels,n_channels):
        ...
        self.class_1_segment_conv = nn.Sequential(
            nn.Conv3d(in_channels+n_channels,out_channels//3,kernel_size=1),
            nn.Sigmoid()
        )
        ...
        self.attention_class_1 = Attention_block(in_channels,n_channels,in_channels//2)
        self.attention_class_2 = Attention_block(in_channels,n_channels,in_channels//2)
        self.attention_class_4 = Attention_block(in_channels,n_channels,in_channels//2)
    ...
    def forward(self, x):
        ...
        class_1_guide = mask_head[:,0:4,:,:,:]
        class_2_guide = mask_head[:,4:8,:,:,:]
        class_4_guide = mask_head[:,8:12,:,:,:]
...
class FNVNSWASRCBGAMHCRFB(nn.Module):
    def __init__(self, n_channels, n_classes,scale=0.5):
        super(FNVNSWASRCBGAMHCRFB, self).__init__()
