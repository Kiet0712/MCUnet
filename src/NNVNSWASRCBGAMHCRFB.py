...
class Up(nn.Module):
    def __init__(self,in_channels,out_channels):
        ...
        self.normalization = nn.InstanceNorm3d(in_channels//2+in_channels//2)
    def forward(self,x1,x2):
        x1 = self.up(x1)
        x2 = self.attention(x1,x2)
        x = self.normalization(torch.cat([x1,x2],dim=1))
        return self.conv(x)
...
class NNVNSWASRCBGAMHCRFB(nn.Module):
    def __init__(self, n_channels, n_classes,scale=0.5):
        super(NNVNSWASRCBGAMHCRFB, self).__init__()
