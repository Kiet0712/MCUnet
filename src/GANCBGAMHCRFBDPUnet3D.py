...
class Attention_block(nn.Module):
    def __init__(self,F_g,F_l,F_int):
        ...
        self.relu_ = nn.LeakyReLU(0.2,inplace=True)
...
class DoubleConv(nn.Module):
    ...
    def forward(self,x):
        return F.leaky_relu(self.double_conv(x)+self.conv_skip_connection(x),0.2)
...
class MP1(nn.Module):
    ...
    def forward(self,x1,x2,x3,x4):
        ...
        return F.leaky_relu(torch.cat([x_1_path,x_2_path,x_3_path,x_4_path],dim=1),0.2)
class MP2(nn.Module):
...
class CRFB(nn.Module):
    ...
    def forward(self,top,bottom):
        return F.leaky_relu(self.deconv(bottom)+top,0.2),F.leaky_relu(self.conv(top)+bottom,0.2)
...
class CBGAMHCRFBDMPUnet3D(nn.Module):
    def __init__(self, n_channels, n_classes,scale=0.5):
        super(CBGAMHCRFBDMPUnet3D, self).__init__()
